const storyData = {
    "StartScreen": {
        isTitleScreen: true,
        content: `
            <h1 class="story-title">Corridor2193 v0.01c - Fase 4</h1>
    ...
    "DiagnosiArmatura": {
        onEnter: function(gs) {
            if (!gs.variables.diagnosiEseguitaQuestoTurno) { // Evita di aumentare lo stress se si torna qui subito
                gs.variables.protagonistaStress = Math.min(10, (gs.variables.protagonistaStress || 0) + 1);
                gs.variables.diagnosiEseguitaQuestoTurno = true; // Flag per evitare stress multiplo
            }
            // gs.variables.armaturaDanneggiata = (Math.random() < 0.4); // 40% probabilità armatura danneggiata
            // gs.variables.feritaGrave = (Math.random() < 0.3 && gs.variables.armaturaDanneggiata); // 30% di ferita grave SE armatura è danneggiata
        },
        contentFunction: function(gs) {
            let html = `
                <p>L'armatura, quanto di più protettivo il genio umano dedicato alla morte poteva inventare. Un misto perfetto di <button type="button" class="inline-link" data-passage-link="desc_dotazione">tecnologia, armamento e resistenza</button>, applicate alla velocità e all'agilità. Un esempio della sua potenza distruttiva era il suo fidato <button type="button" class="inline-link" data-passage-link="desc_fucile">fucile al plasma</button>, inseparabile compagno. Anche per quello si era fermato, anche per controllare i suoi danni o ferite. Era necessario e lo sapeva, l'adrenalina aveva ancora un effetto negativo negli uomini costruiti per devastare. Spesso non ci si accorgeva di essere feriti oppure capitava che lo si dimenticasse.</p>
                <p>Prese fiato, cercò di regolarizzare il suo organismo, si nutrì rapidamente con quella <button type="button" class="inline-link" data-passage-link="desc_cibo">merda liofilizzata iper proteinica</button> che aveva in dotazione, mentre il sistema di diagnosi scorreva i suoi controlli.</p>
                <p class='diagnosi-risultato'>SCANSIONE SISTEMI IN CORSO...</p>
            `;
            
            // Logica per i risultati della diagnosi
            let scudi = 100 - ((gs.variables.protagonistaStress || 0) * 5 + (gs.variables.adrenalinaSinteticaUsata || 0) * 3); // Calcolo scudi
            scudi = Math.max(10, scudi); // Minimo 10% scudi
            gs.variables.scudiAttuali = scudi;

            let statoArmatura = "Operativa.";
            let statoBiologico = "Parametri vitali stabili.";
            let necessitaCure = false;

            if (gs.variables.scudiAttuali < 50) {
                statoArmatura = "<span style='color:var(--accent-warning);'>Scudi al " + gs.variables.scudiAttuali + "%. Integrità strutturale compromessa in alcuni settori.</span>";
            } else {
                statoArmatura = "<span style='color:var(--accent-tech);'>Scudi al " + gs.variables.scudiAttuali + "%. Tutti i sistemi primari operativi.</span>";
            }

            // Simulazione ferita (più probabile se stress/adrenalina alti)
            if (!gs.variables.feritaTrattata && ((gs.variables.protagonistaStress || 0) > 6 || (gs.variables.adrenalinaSinteticaUsata || 0) > 2) && Math.random() < 0.4) {
                gs.variables.feritaAttiva = true;
                statoBiologico = "<span style='color:var(--accent-danger);'>ALLARME BIOLOGICO: Rilevata perforazione minore corazza, emorragia interna di livello 2 nel settore toracico destro. Intervento medico immediato consigliato!</span>";
                necessitaCure = true;
                 if (gs.variables.echoPresente) {
                    html += `<div class="echo-communication">Alpha-7, la diagnostica segnala una ferita! Non l'avevi notata? L'adrenalina può fare brutti scherzi. Devi stabilizzarti!</div>`;
                }
            } else if (gs.variables.feritaAttiva) { // Se la ferita c'era già e non è stata curata
                statoBiologico = "<span style='color:var(--accent-danger);'>ALLARME BIOLOGICO: Emorragia interna non trattata! Condizioni in peggioramento!</span>";
                necessitaCure = true;
            }


            html += `
                <p class='diagnosi-risultato'>DIAGNOSI COMPLETATA:</p>
                <ul class='diagnosi-lista'>
                    <li>STATO ARMATURA: ${statoArmatura}</li>
                    <li>STATO BIOLOGICO: ${statoBiologico}</li>
                </ul>
            `;
            
            if (gs.variables.feritaAttiva && !gs.variables.medikitExtra) {
                html += "<p><span style='color:var(--accent-warning);'>Hai utilizzato il tuo unico medikit di emergenza in dotazione. Non ne hai altri.</span></p>";
            }
            
            html += "<p>Lasciò passare qualche istante prima di decidere il da farsi.</p>";
            delete gs.variables.diagnosiEseguitaQuestoTurno; // Resetta il flag
            return html;
        },
        choicesFunction: function(gs) {
            const choices = [];
            if (gs.variables.feritaAttiva && gs.variables.medikitExtra) {
                choices.push({ text: "Usa un medikit extra per curare la ferita.", target: "UsaMedikitExtra_Azione" }); // Nuovo Passaggio
            } else if (gs.variables.feritaAttiva && !gs.variables.medikitExtra) {
                choices.push({ text: "(Ferito e senza medikit) Stringi i denti e vai avanti.", target: "ProntoAllAzione" });
            }
            
            choices.push({ text: "Ignora (o hai già gestito) i danni e preparati all'azione.", target: "ProntoAllAzione" });
            return choices;
        }
    },
    "ProntoAllAzione": {
        onEnter: function(gs) {
            // Resetta flag per evitare stress multiplo se si torna qui da DiagnosiArmatura
            delete gs.variables.diagnosiEseguitaQuestoTurno;
            if ((gs.variables.protagonistaStress || 0) > 6) {
                gs.variables.protagonistaStress = Math.min(10, (gs.variables.protagonistaStress || 0) + 1); // La tensione prima dell'azione
            }
        },
        contentFunction: function(gs) {
            let html = "";
            if ((gs.variables.protagonistaStress || 0) > 7) {
                html += "<p><em>Il cuore gli martellava nel petto come un maglio impazzito, ogni respiro un sibilo affannoso. Ogni ombra si contorceva, trasformandosi in un nemico in agguato; ogni suono metallico lontano era un passo inesorabile verso la sua fine. 'Calma,' si impose con fatica, 'respira. La paura uccide più rapidamente dei proiettili nemici.'</em></p>";
            } else if ((gs.variables.protagonistaStress || 0) > 4) {
                 html += "<p><em>Un'inquietudine fredda gli serpeggiava nelle vene, un presagio di scontro imminente. Cercò di scacciare i pensieri negativi, concentrandosi sui protocolli di combattimento.</em></p>";
            }

            if (gs.variables.echoPresente && (gs.variables.echoRapporto || 0) > 1) {
                html += `<div class="echo-communication">Movimenti nemici confermati in avvicinamento rapido dalla tua ultima posizione nota. Qualunque cosa tu debba fare, falla in fretta. Sembra che abbiano ristretto il campo. Preparati a un contatto imminente.</div>`;
            } else if (gs.variables.echoPresente) {
                 html += `<div class="echo-communication">Attività ostile rilevata. Massima allerta.</div>`;
            }
            html += `
                <p>Si cominciarono a sentire passi pesanti di grosse armature, qualche ruggito disumano e i versi stridenti dei mostruosi esseri biomeccanici che lo stavano inseguendo. Esplosioni sorde accompagnavano la loro marcia, e il caratteristico ronzio di un'unità pesante – probabilmente una di quelle ultra corazzate, una sorta di drone gigante con innesti d'arma micidiali – si faceva sempre più vicino.</p>
                <p>Ora il suo nascondiglio, quel piccolo angolo d'ombra che lo teneva fuori dalla luce e dalla vista, non sarebbe più bastato. Avevano cercato un po' ovunque, con una metodicità brutale, ma lui si era imboscato bene. Precedentemente vide qualche nemico passare davanti al suo rifugio, ma si guardò bene dal farsi scoprire (<button type="button" class="inline-link" data-passage-link="desc_nemico">attaccare</button> sarebbe stato da puri folli in quella situazione). Non lo avevano ancora visto e quindi non era utile dare un segnale della sua presenza.</p>
                <p>Questa volta la <button type="button" class="inline-link" data-passage-link="desc_ricerca">ricerca era molto accurata</button>, quasi personale. Stavano impiegando una squadra di soldati d'assalto, almeno un ufficiale superiore – a giudicare dalle comunicazioni radio nemiche intercettate a sprazzi – e quel grosso drone da combattimento. Poteva intuirlo dai suoni distinti e dalle voci filtrate che sentiva. Stavano sparando cannonate esplosive in ogni angolo o pertugio che potesse nasconderlo. Era l'ultimo della sua truppa, un boccone solitario e prezioso per i suoi cacciatori, e prima o poi sarebbe sicuramente morto... almeno che non fosse stato talmente agile, o fortunato, da fuggire tanto velocemente da poter sopravvivere un altro maledetto giorno.</p>
            `;
            return html;
        },
        choices: [
            { text: "Non c'è più tempo per pensare, devi uscire e affrontare la situazione!", target: "SecondaParte" }
        ]
    },
    "SecondaParte": {
        onEnter: function(gs) {
    ...
    "credits": {
        ...
        content: `
            <div style="text-align: center;">
                <h2>Corridor2193 v0.01c - Fase 4</h2>
                <p>Narrativa interattiva sci-fi ispirata alle partite di Doom e Quake degli anni '90.</p>
                <p>Storia originale "La Fuga" di Simone Pizzi.</p>
                <p>Sviluppo ed espansione interattiva coadiuvati da LLM.</p>
                <br>
                <p>Versione 0.01c - Fase 4</p>
                <p>IFID Originale Storia: 855043D3-FCC4-42C6-A3A3-CE24379D7BF2 (per riferimento)</p>
            </div>
        `,
        choices: [
            { text: "GIOCA ANCORA", target: "StartScreen", classes: "start-button credits-button" }
        ]
    },
    "UsaMedikitExtra_Azione": {
        onEnter: function(gs) {
            if (gs.variables.medikitExtra && gs.variables.feritaAttiva) {
                gs.variables.medikitExtra = false; // Medikit usato
                gs.variables.feritaAttiva = false;  // Ferita curata
                gs.variables.feritaTrattata = true; // Flag per non farla ricomparire subito
                gs.variables.protagonistaStress = Math.max(0, (gs.variables.protagonistaStress || 0) - 3); // Sollievo significativo
                console.log("Medikit extra usato. Ferita curata.");
            } else {
                console.warn("Tentativo di usare medikit extra senza averlo o senza ferita attiva.");
            }
        },
        content: `
            <p>Con mani più salde di quanto ti aspettassi, estrai il medikit extra che avevi recuperato. Il gel autosterilizzante e i nanocomponenti riparatori fanno rapidamente il loro lavoro sulla ferita al fianco. Il dolore si attenua quasi subito, sostituito da un leggero formicolio e dal calore della rigenerazione tissutale.</p>
            <p>Ti senti decisamente meglio, più lucido e pronto ad affrontare ciò che verrà.</p>
        `,
        choices: [
            { text: "Ora sei veramente pronto all'azione.", target: "ProntoAllAzione" }
        ]
    },
    "Seduto_Finale_Considera": {
        content: `
...
}; // QUESTA è la chiusura dell'intero oggetto storyData.

function getDescriptionChoices(gs) {
    let returnToPassageId = "AppoggiatoAlMuro"; // Default fallback

    if (gs.variables.descriptionContext && gs.variables.descriptionContext.length > 0) {
        // Prende l'ID del passaggio da cui è stata chiamata la descrizione
        returnToPassageId = gs.variables.descriptionContext[gs.variables.descriptionContext.length - 1].fromPassage;
            } else {
        // Fallback se descriptionContext è vuoto o non definito
...
// Applica dinamicamente le scelte di ritorno ai passaggi descrittivi e aggiunge ID
for (const passageId in storyData) {
    if (storyData.hasOwnProperty(passageId)) {
        const passage = storyData[passageId];
        if (passage.isDescription) {
            if (!passage.choices || (Array.isArray(passage.choices) && passage.choices.length === 0)) {
                passage.choices = (gameState) => getDescriptionChoices(gameState);
            }
        }
        // Aggiungi l'ID del passaggio all'oggetto del passaggio stesso per comodità
